...
output_file = "menus_cantines_pretraite_v3.csv"
...
# --- FILTRER LES JOURS INUTILES OU MUETS EN TERMES D'INFORMATIONS DE DONNEES ---
...
# 1) Ajouter les colonnes "Année" (calendaire), "AnnéeISO" et "Semaine" (ISO)
df["Année"] = df["Date"].dt.year

# récupérer isocalendar (obtenir année ISO et semaine ISO de manière cohérente)
isocal = df["Date"].dt.isocalendar()
df["AnnéeISO"] = isocal.year
df["Semaine"] = isocal.week

# 2) Grouper par AnnéeISO et Semaine pour vérifier s'il y a un plat végé dans la semaine
weekly_vege = df.groupby(["AnnéeISO", "Semaine"])["is_vege_day"].any().reset_index()
...
# Maintenant chaque ligne a une colonne True/False si la semaine (ISO) contient au moins un repas végé
